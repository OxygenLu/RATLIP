...
class CLIP_IMG_ENCODER(nn.Module):
    ...
    def forward(self, img: torch.Tensor):
        ...
        for i in range(12):
            ...
            if i in selected:
                local_features.append(x.permute(1, 0, 2)[:, 1:, :].\
                                      permute(0, 2, 1).reshape(-1, 768, grid, grid).\
                                      contiguous().type(img.dtype))
    ...
    def forward(self, img: torch.Tensor, prompts: torch.Tensor):
        ...
        for i in range(begin, end):
            if i in selected:
                ...
                x = self.transformer.resblocks[i](x)
        return  x.permute(1, 0, 2)[:, 1:, :].permute(0, 2, 1).\
                reshape(-1, 768, grid, grid).contiguous().type(img.dtype)
...
class CLIP_Adapter(nn.Module):
    ...
    def __init__(self, in_ch, mid_ch, out_ch, G_ch, CLIP_ch, 
        ...
        self.FBlocks.append(M_Block_RAT(in_ch, mid_ch, out_ch, 
                                        cond_dim, k, s, p, hidden_size))# 加了rat
        for i in range(map_num-1):
            self.FBlocks.append(M_Block_RAT(out_ch, mid_ch, out_ch, 
                                            cond_dim, k, s, p, hidden_size))# 加了rat
...
class NetG(nn.Module):
    ...
    def forward(self, noise, c, eval=False): # x=noise, c=ent_emb
        with torch.cuda.amp.autocast() if self.mixed_precision and \
                                            not eval else dummy_context_mgr() as mp:
